...
Notice the possible outliers not shown in the previous figure.
```{r, echo=FALSE}
# Sort literature by TimesCited in decreasing order and display top 10
options(width = 140)
mostCited <- head(literature[with (literature, order(-TimesCited)), ], 10)
mostCited$ReferenceString <- strtrim(mostCited$ReferenceString, 100)
mostCited[, c("ReferenceString", "TimesCited")]
```

## Times cited by year published
```{r,echo=FALSE}
# Set x-axis labels manually
# if range < 10, show all, otherwise every 5th year
start <- min(literature$YearPublished, na.rm = T)
end <- max(literature$YearPublished, na.rm = T)
yearRange <- end - start
steps <- 1
if (yearRange > 10) {steps <- 5} 

ggplot(literature, aes(YearPublished, TimesCited, 
                       color = PublicationType)) + 
    geom_point(position = "jitter", alpha = 1/5) + 
    ggtitle("Citations by year published") + 
    scale_x_continuous(breaks =  seq(start, end, steps)) +
    xlab("Year") + 
    ylab("Times cited")
```
...
ggplot(head(publications, 25), aes(Publication, Count)) + 
    geom_bar(stat = "identity", aes(fill = Publication)) + 
    coord_flip() + 
...
ggplot(top25Lit, 
       aes(PublicationName, TimesCited, 
           color = PublicationName, color = PublicationName)) + 
    geom_boxplot() + 
    coord_flip() +
    theme(legend.position = "none") +
...
## Most cited papers within the dataset
```{r,echo=FALSE}
# Calculate and display ratios of papers and references with DOIs

DOIratio <- round((1 - sum(literature$DOI == "") / nrow(literature)) * 100, 2)
references <- unlist(strsplit(literature$CitedReferences, ";"))
refRatio <- round(nrow(citationEdges) / length(references), 2)
cat(DOIratio, "% of papers and ", refRatio, "% of references contain DOIs.")
```
