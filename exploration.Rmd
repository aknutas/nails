...
kable(mostCited[, c("FullReference", "TimesCited")])
```
...
citationNodes$EigenvectorCentrality <- evcent(citationGraph, directed = TRUE)$vector
...
citationsLit <- merge(citationsLit, literature[, c(2, 11, 23, 62)], 
...
kable(head(citationsLit[, c("FullReference", "InDegree")], 25))
```
...
kable(head(citationsRef[, c("FullReference", "InDegree")], 25))
```
...
kable(head(citationsLit[, c("FullReference", "PageRank")], 25))
```
...
kable(head(citationsRef[, c("FullReference", "PageRank")], 25))
```

### The articles with the highest eigenvector centralities
```{r,echo=FALSE}
# Order by eigenvector centrality, decreasing order
citationsLit <- citationsLit[with (citationsLit, order(-EigenvectorCentrality)), ]
citationsRef <- citationsRef[with (citationsRef, order(-EigenvectorCentrality)), ]
# Display the top 25 papers with highest eigenvector centrality included in the dataset 
kable(head(citationsLit[, c("FullReference", "EigenvectorCentrality")], 25))
```

### The references with the highest eigenvector centralities NOT found in the dataset
```{r,echo=FALSE}
# Display the top 25 papers with highest eigenvector centrality not included in the dataset 
kable(head(citationsRef[, c("FullReference", "EigenvectorCentrality")], 25))
```

# Abstracts
## Highest in-degree
```{r,echo=FALSE}
# Helper function for printing abstracts
printAbstract <- function(df) {
    title <- toupper(df[10])
    abstract <- df[11]
    cat(paste(title, "\n", abstract, "\n\n"))
}
citationsLit <- citationsLit[with (citationsLit, order(-InDegree)), ]
apply(head(citationsLit, 10), 1, printAbstract)
```
